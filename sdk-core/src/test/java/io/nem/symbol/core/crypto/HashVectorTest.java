...
class HashVectorTest extends AbstractVectorTester {
  ...
  private static Stream<Arguments> testSha256() throws Exception {
    return createArguments("0.test-sha3-256.json", HashVectorTest::extractArguments, 10);
  }

  private static Stream<Arguments> testKeccak256() throws Exception {
    return createArguments("0.test-keccak-256.json", HashVectorTest::extractArguments, 10);
  }

  private static List<Arguments> extractArguments(Map<String, String> entry) {
    return Collections.singletonList(
        Arguments.of(entry.get("data"), entry.get("hash"), entry.get("length")));
  }
  ...
  void testSha256(String data, String hash, int length) {
    ...
    Assertions.assertEquals(
        hash.toUpperCase(), ConvertUtils.toHex(Hashes.sha3_256(decode)).toUpperCase());
  }
  ...
  void testKeccak256(String data, String hash, int length) {
    ...
    Assertions.assertEquals(length, decode.length);
    Assertions.assertEquals(
        hash.toUpperCase(), ConvertUtils.toHex(Hashes.keccak256(decode)).toUpperCase());
  }
}
