...
public class MathUtils {
  ...
  /**
   * Calculates (a * b + c) mod group order and returns the result. a, b and c are given in 2^8 bit
   * representation.
   *
  ...
  /**
   * Gets a random encoded group element. It's NOT guaranteed that the created encoded group element
   * is a multiple of the base point.
   *
  ...
  /**
   * Creates a group element from a byte array. Bit 0 to 254 are the affine y-coordinate, bit 255 is
   * the sign of the affine x-coordinate.
   *
  ...
  /**
   * Converts a group element from one coordinate system to another. This method is a helper used to
   * test various methods in Ed25519GroupElement.
   *
    ...
    switch (newCoordinateSystem) {
      ...
      case CACHED:
        return Ed25519GroupElement.cached(
            ...
            Ed25519Field.ONE,
            toFieldElement(
                D.multiply(new BigInteger("2")).multiply(x).multiply(y).mod(Ed25519Field.P)));
      case PRECOMPUTED:
  ...
  /**
   * Doubles a group element and returns the result in the P3 coordinate system. It uses BigInteger
   * arithmetic and the affine coordinate system. This method is a helper used to test the
   * projective group doubling formula in Ed25519GroupElement.
   *
   * @param g The group element.
   * @return g+g.
   */
  ...
  public static Signature sign(final KeyPair keyPair, final byte[] data) {
    ...
    final Ed25519EncodedFieldElement r =
        new Ed25519EncodedFieldElement(hasher64.hash(Arrays.copyOfRange(hash, 32, 64), data));
    final Ed25519EncodedFieldElement rReduced = reduceModGroupOrder(r);
    final Ed25519GroupElement R =
        scalarMultiplyGroupElement(Ed25519Group.BASE_POINT, toFieldElement(toBigInteger(rReduced)));
    final Ed25519EncodedFieldElement h =
        new Ed25519EncodedFieldElement(
            hasher64.hash(R.encode().getRaw(), keyPair.getPublicKey().getBytes(), data));
