...
public class KeyPairVectorTester extends AbstractVectorTester {
  ...
  private static Stream<Arguments> testKeys() {
    return createArguments("1.test-keys.json", KeyPairVectorTester::extractArguments, 10);
  }

  private static List<Arguments> extractArguments(Map<String, String> entry) {
    return Collections.singletonList(Arguments.of(entry.get("privateKey"), entry.get("publicKey")));
  }
  ...
  void testKeys(String privateKey, String publicKey) {
    ...
    Assertions.assertEquals(publicKey.toUpperCase(), keyPair.getPublicKey().toHex().toUpperCase());
  }
}
