...
public class Ed25519BlockCipherTest extends BlockCipherTest {
  ...
  public void decryptFailsInputIsTooSmallInLength() {
    ...
    CryptoException exception =
        Assertions.assertThrows(CryptoException.class, () -> blockCipher.decrypt(new byte[27]));

    // Assert:
    Assertions.assertEquals(
        "Cannot decrypt input. Size is 27 when at least 28 is expected.", exception.getMessage());
  }
  ...
  public void decryptFailsIfInputIsNull() {
    ...
    // Act:
    CryptoException exception =
        Assertions.assertThrows(CryptoException.class, () -> blockCipher.decrypt(null));
