...
class MosaicVectorTester extends AbstractVectorTester {
    ...
    private static Stream<Arguments> testMosaics() {
        return createArguments("5.test-mosaic-id.json", MosaicVectorTester::extractArgumentsSha,
            50
        );
    }
    ...
    private static List<Arguments> extractArgumentsSha(Map<String, String> entry) {
        ...
        arguments.add(extractArguments(entry, NetworkType.MAIN_NET, "address_Public", "mosaicId_Public"));
        arguments.add(extractArguments(entry, NetworkType.TEST_NET, "address_PublicTest", "mosaicId_PublicTest"));
        arguments.add(extractArguments(entry, NetworkType.MIJIN, "address_Mijin", "mosaicId_Mijin"));
        arguments.add(extractArguments(entry, NetworkType.MIJIN_TEST, "address_MijinTest", "mosaicId_MijinTest"));
        return arguments;
    }
    ...
    void testMosaics(NetworkType networkType, long mosaicNonce, String plain, String mosaicHex) {
        ...
        MosaicId mosaicId = MosaicId
            .createFromNonce(MosaicNonce.createFromBigInteger(BigInteger.valueOf(mosaicNonce)), address);
