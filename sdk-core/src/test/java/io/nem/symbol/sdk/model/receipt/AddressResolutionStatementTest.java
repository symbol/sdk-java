...
public class AddressResolutionStatementTest {
    ...
    addressResolutionStatements =
        Collections.singletonList(
            new AddressResolutionStatement(
                ...
                Collections.singletonList(
                    new ResolutionEntry<>(
                        address1, new ReceiptSource(1, 0), ReceiptType.ADDRESS_ALIAS_RESOLUTION))));
  }
  ...
  void shouldGetResolvedEntryWhenPrimaryIdIsGreaterThanMaxAddress() {
    Optional<Address> resolved =
        AddressResolutionStatement.getResolvedAddress(
            addressResolutionStatements, height, addressNamespace1, 4, 0);
  ...
  void shouldNotResolveAddressWhenInvalidHeight() {
    Optional<Address> resolved =
        AddressResolutionStatement.getResolvedAddress(
            addressResolutionStatements, height2, addressNamespace1, 4, 0);
  ...
  void shouldGetResolvedEntryRealAddress() {
    Optional<Address> resolved =
        AddressResolutionStatement.getResolvedAddress(
            addressResolutionStatements, height, address1, 4, 0);
    Assertions.assertTrue(resolved.isPresent());
    Assertions.assertEquals(address1, resolved.get());
  }
  ...
  void shouldGetResolvedEntryWhenPrimaryIdMatchesButSecondaryIdLessThanMinimum() {
    Optional<Address> resolved =
        AddressResolutionStatement.getResolvedAddress(
            addressResolutionStatements, height, mosaicNamespace1, 0, 6);
    Assertions.assertFalse(resolved.isPresent());
  }
  ...
  void shouldReturnUndefinedAddress() {
    Optional<Address> resolved =
        AddressResolutionStatement.getResolvedAddress(
            addressResolutionStatements, height, addressNamespace1, 0, 6);
    Assertions.assertFalse(resolved.isPresent());
  }
  ...
  void resolutionChangeInTheBlockMoreThanOneAggregate() {
    Optional<Address> resolved =
        AddressResolutionStatement.getResolvedAddress(
            addressResolutionStatements, height2, addressNamespace1, 0, 6);
    Assertions.assertFalse(resolved.isPresent());
  }
}
