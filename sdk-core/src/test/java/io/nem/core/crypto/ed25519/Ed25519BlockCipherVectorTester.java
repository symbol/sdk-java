...
class Ed25519BlockCipherVectorTester extends AbstractVectorTester {
    ...
    void testResolveSharedKey(SignSchema signSchema, String salt, String privateKey,
        ...
        byte[] resolvedScalarMulResult = Ed25519BlockCipher.sharedKeyNotSalted(
            privateKeyObject,
            otherPublicKeyObject,
            signSchema);
        Assertions.assertEquals(sharedKey, Hex.toHexString(resolvedSharedKey));
        Assertions.assertEquals(scalarMulResult, Hex.toHexString(resolvedScalarMulResult));
    }
}
