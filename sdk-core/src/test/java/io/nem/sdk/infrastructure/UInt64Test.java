...
class UInt64Test {
    ...
    @ParameterizedTest
    @MethodSource("provider")
    void testUInt64FromLongArray(int[] input, BigInteger expected) {
        long[] inputAsLongs = Arrays.stream(input).mapToLong(i -> i).toArray();
        BigInteger result = UInt64.fromLongArray(inputAsLongs);
        assertEquals(expected, result);
    }

    @ParameterizedTest
    @MethodSource("provider")
    void testUInt64FromNumberList(int[] input, BigInteger expected) {
        BigInteger result = UInt64
            .extractBigInteger(
                Arrays.stream(input).mapToObj(Long::valueOf).collect(Collectors.toList()));
        ;
        assertEquals(expected, result);
    }
}
