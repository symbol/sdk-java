...
public class NetworkCurrencyServiceImpl implements NetworkCurrencyService {
  ...
  public Observable<List<NetworkCurrency>> getNetworkCurrenciesFromNemesis() {
    return getBlockTransactions(BigInteger.ONE, 1)
        .map(
            transactions -> {
              List<MosaicDefinitionTransaction> mosaicTransactions =
                  transactions.stream()
                      .filter(t -> t.getType() == TransactionType.MOSAIC_DEFINITION)
                      .map(t -> (MosaicDefinitionTransaction) t)
                      .collect(Collectors.toList());

              List<MosaicAliasTransaction> aliasTransactions =
                  transactions.stream()
                      .filter(t -> t.getType() == TransactionType.MOSAIC_ALIAS)
                      .map(t -> (MosaicAliasTransaction) t)
                      .collect(Collectors.toList());

              List<NamespaceRegistrationTransaction> namespaceRegistrations =
                  transactions.stream()
                      .filter(t -> t.getType() == TransactionType.NAMESPACE_REGISTRATION)
                      .map(t -> (NamespaceRegistrationTransaction) t)
                      .collect(Collectors.toList());

              Stream<Stream<NetworkCurrency>> streamStream =
                  mosaicTransactions.stream()
                      .map(
                          mosaicTransaction -> {
                            List<MosaicAliasTransaction> mosaicAliasTransactions =
                                aliasTransactions.stream()
                                    .filter(
                                        a ->
                                            a.getMosaicId().equals(mosaicTransaction.getMosaicId()))
                                    .collect(Collectors.toList());
                            return mosaicAliasTransactions.stream()
                                .map(
              ...
              return streamStream.flatMap(Function.identity()).collect(Collectors.toList());
            });
  }
  ...
  public Observable<NetworkCurrency> getNetworkCurrencyFromMosaicId(MosaicId mosaicId) {
    ...
    return this.mosaicRepository
        ...
        .flatMap(
            info -> {
              Observable<Optional<NamespaceId>> namespaceIdObservable =
                  this.namespaceRepository
                      ...
                      .map(
                          mosaicNames -> {
                            ...
                            return Optional.of(
                                NamespaceId.createFromName(
                                    mosaicNames.getNames().get(0).getName()));
                          })
                      .first(Optional.empty())
                      .onErrorReturnItem(Optional.empty())
                      .toObservable();

              return namespaceIdObservable.map(
                  namespaceIdOptional ->
                      createNetworkCurrencyBuilder(
                              mosaicId, namespaceIdOptional.orElse(null), info.getDivisibility())
                          .withSupplyMutable(info.isSupplyMutable())
                          .withTransferable(info.isTransferable())
                          .build());
            });
  }
            ...
            mosaicId ->
                this.mosaicRepository
                    ...
                    .map(
                        info ->
                            createNetworkCurrencyBuilder(
                                    mosaicId, namespaceId, info.getDivisibility())
                                .withSupplyMutable(info.isSupplyMutable())
                                .withTransferable(info.isTransferable())
                                .build()));
  }
  ...
  private Optional<NetworkCurrency> getNetworkCurrency(
    ...
    Optional<String> namespaceNameOptional =
        getNamespaceFullName(namespaceRegistrations, mosaicAliasTransaction.getNamespaceId());
    return namespaceNameOptional.map(
        namespaceName -> {
          NamespaceId namespaceId =
              NamespaceId.createFromIdAndFullName(
                  mosaicAliasTransaction.getNamespaceId().getId(), namespaceName);

          NetworkCurrencyBuilder builder =
              createNetworkCurrencyBuilder(
                  mosaicId, namespaceId, mosaicTransaction.getDivisibility());
  ...
  private Optional<String> getNamespaceFullName(
    ...
    // If the fullname is already in the NamespaceId, we can shortcut the
    // processing.
    if (namespaceId.getFullName().isPresent()) {
      return namespaceId.getFullName();
    }
    Optional<NamespaceRegistrationTransaction> namespaceOptional =
        transactions.stream().filter(tx -> tx.getNamespaceId().equals(namespaceId)).findFirst();
    return namespaceOptional.flatMap(
        childNamespace -> {
          if (childNamespace.getNamespaceRegistrationType()
            ...
            return childNamespace
                ...
                .flatMap(
                    parentId -> {
                      Optional<String> parentNamespaceNameOptional =
                          getNamespaceFullName(transactions, parentId);
                      return parentNamespaceNameOptional.map(
                          parentNamespaceName ->
                              parentNamespaceName + "." + childNamespace.getNamespaceName());
                    });
          }
        });
  }
  ...
  /**
   * It returns all the transactions for the given block starting from the given transaction id. If
   * the fromId, it will start from the start.
   *
   * <p>This method is recursive, moving thorough all the pages.
   *
            ...
            page -> {
              if (page.getPageNumber() <= pageNumber) {
                ...
                return getBlockTransactions(height, pageNumber + 1)
                    .map(
                        tail ->
                            Stream.concat(page.getData().stream(), tail.stream())
                                .collect(Collectors.toList()));
              }
            });
  }

  private NetworkCurrencyBuilder createNetworkCurrencyBuilder(
      MosaicId mosaicId, NamespaceId namespaceId, int divisibility) {

    Validate.isTrue(
        mosaicId != null || namespaceId != null,
    ...
    NetworkCurrencyBuilder builder = new NetworkCurrencyBuilder(unresolvedMosaicId, divisibility);
    if (mosaicId != null) {
