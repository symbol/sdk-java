 ...
 * the ref10 implementation of SUPERCOP. <br>
 * Literature: [1] Daniel J. Bernstein, Niels Duif, Tanja Lange, Peter Schwabe and Bo-Yin Yang :
 * High-speed high-security signatures [2] Huseyin Hisil, Kenneth Koon-Ho Wong, Gary Carter, Ed
 * Dawson: Twisted Edwards Curves Revisited [3] Daniel J. Bernsteina, Tanja Lange: A complete set of
 * addition laws for incomplete Edwards curves [4] Daniel J. Bernstein, Peter Birkner, Marc Joye,
 * Tanja Lange and Christiane Peters: Twisted Edwards Curves [5] Christiane Pascale Peters: Curves,
 * Codes, and Cryptography (PhD thesis) [6] Daniel J. Bernstein, Peter Birkner, Tanja Lange and
 * Christiane Peters: Optimizing double-base elliptic-curve single-scalar multiplication
 */
public class Ed25519GroupElement implements Serializable {
   ...
   * learn more about it see [6] page 8. <br>
   * Output: r which satisfies a = r0 * 2^0 + r1 * 2^1 + ... + r255 * 2^255 with ri in {-15, -13,
   * -11, -9, -7, -5, -3, -1, 0, 1, 3, 5, 7, 9, 11, 13, 15} <br>
   ...
   * Supported conversions: - P3 -> P2 - P3 -> CACHED (1 multiply, 1 add, 1 subtract) - P1xP1 -> P2
   * (3 multiply) - P1xP1 -> P3 (4 multiply)
   *
  ...
  private Ed25519GroupElement toCoordinateSystem(final CoordinateSystem newCoordinateSystem) {
    switch (this.coordinateSystem) {
      ...
      case PRECOMPUTED:
        if (newCoordinateSystem == CoordinateSystem.PRECOMPUTED) { // noinspection
    ...
    for (int i = 0; i < 32; i++) {
      ...
      for (int j = 0; j < 8; j++) {
        ...
        this.precomputedForSingle[i][j] =
            precomputed(
                yElement.add(xElement),
                yElement.subtract(xElement),
  ...
  /**
   * Doubles a given group element p in P^2 or P^3 coordinate system and returns the result in P x P
   * coordinate system. r = 2 * p where p = (X : Y : Z) or p = (X : Y : Z : T) <br>
   * r in P x P coordinate system: <br>
   * r = ((X' : Z'), (Y' : T')) where X' = (X + Y)^2 - (Y^2 + X^2) Y' = Y^2 + X^2 Z' = y^2 - X^2 T'
   * = 2 * Z^2 - (y^2 - X^2) <br>
   * r converted from P x P to P^2 coordinate system: <br>
   * r = (X'' : Y'' : Z'') where X'' = X' * T' = ((X + Y)^2 - Y^2 - X^2) * (2 * Z^2 - (y^2 - X^2))
   * Y'' = Y' * Z' = (Y^2 + X^2) * (y^2 - X^2) Z'' = Z' * T' = (y^2 - X^2) * (2 * Z^2 - (y^2 - X^2))
   * <br>
   * Formula for the P^2 coordinate system is in agreement with the formula given in [4] page 12
   * (with a = -1) (up to a common factor -1 which does not matter): <br>
   * B = (X + Y)^2; C = X^2; D = Y^2; E = -C = -X^2; F := E + D = Y^2 - X^2; H = Z^2; J = F − 2 * H;
   * X3 = (B − C − D) · J = X' * (-T'); Y3 = F · (E − D) = Z' * (-Y'); Z3 = F · J = Z' * (-T').
   *
   ...
   * g where this = (X1 : Y1 : Z1 : T1), g = (g.X, g.Y, g.Z) = (Y2/Z2 + X2/Z2, Y2/Z2 - X2/Z2, 2 * d
   * * X2/Z2 * Y2/Z2) <br>
   * r in P x P coordinate system: <br>
   * r = ((X' : Z'), (Y' : T')) where X' = (Y1 + X1) * g.X - (Y1 - X1) * q.Y = ((Y1 + X1) * (Y2 +
   * X2) - (Y1 - X1) * (Y2 - X2)) * 1/Z2 Y' = (Y1 + X1) * g.X + (Y1 - X1) * q.Y = ((Y1 + X1) * (Y2 +
   * X2) + (Y1 - X1) * (Y2 - X2)) * 1/Z2 Z' = 2 * Z1 + T1 * g.Z = 2 * Z1 + T1 * 2 * d * X2 * Y2 *
   * 1/Z2^2 = (2 * Z1 * Z2 + 2 * d * T1 * T2) * 1/Z2 T' = 2 * Z1 - T1 * g.Z = 2 * Z1 - T1 * 2 * d *
   * X2 * Y2 * 1/Z2^2 = (2 * Z1 * Z2 - 2 * d * T1 * T2) * 1/Z2 <br>
   * Formula for the P x P coordinate system is in agreement with the formula given in file
   * ge25519.c method add_p1p1() in ref implementation. Setting A = (Y1 - X1) * (Y2 - X2), B = (Y1 +
   * X1) * (Y2 + X2), C = 2 * d * T1 * T2, D = 2 * Z1 * Z2 we get X' = (B - A) * 1/Z2 Y' = (B + A) *
   * 1/Z2 Z' = (D + C) * 1/Z2 T' = (D - C) * 1/Z2 <br>
   * r converted from P x P to P^2 coordinate system: <br>
   * r = (X'' : Y'' : Z'' : T'') where X'' = X' * T' = (B - A) * (D - C) * 1/Z2^2 Y'' = Y' * Z' = (B
   * + A) * (D + C) * 1/Z2^2 Z'' = Z' * T' = (D + C) * (D - C) * 1/Z2^2 T'' = X' * Y' = (B - A) * (B
   * + A) * 1/Z2^2 <br>
   * Formula above for the P^2 coordinate system is in agreement with the formula given in [2] page
   * 6 (the common factor 1/Z2^2 does not matter) E = B - A, F = D - C, G = D + C, H = B + A X3 = E
   * * F = (B - A) * (D - C); Y3 = G * H = (D + C) * (B + A); Z3 = F * G = (D - C) * (D + C); T3 = E
   * * H = (B - A) * (B + A);
   *
   ...
   * Negating g means negating the value of X2 and T2 (the latter is irrelevant here). The formula
   * is in accordance to the above addition.
   *
   ...
   * where this = (X1 : Y1 : Z1 : T1), g = (g.X, g.Y, g.Z, g.T) = (Y2 + X2, Y2 - X2, Z2, 2 * d * T2)
   * <br>
   * r in P x P coordinate system.: X' = (Y1 + X1) * (Y2 + X2) - (Y1 - X1) * (Y2 - X2) Y' = (Y1 +
   * X1) * (Y2 + X2) + (Y1 - X1) * (Y2 - X2) Z' = 2 * Z1 * Z2 + 2 * d * T1 * T2 T' = 2 * Z1 * T2 - 2
   * * d * T1 * T2 <br>
   * Setting A = (Y1 - X1) * (Y2 - X2), B = (Y1 + X1) * (Y2 + X2), C = 2 * d * T1 * T2, D = 2 * Z1 *
   * Z2 we get X' = (B - A) Y' = (B + A) Z' = (D + C) T' = (D - C) <br>
   ...
   * Ed25519GroupElement subtraction using the twisted Edwards addition law for extended
   * coordinates. <br>
   * Negating g means negating the value of the coordinate X2 and T2. The formula is in accordance
   * to the above addition.
   *
  ...
  private Ed25519GroupElement select(final int pos, final int b) {
    ...
    final Ed25519GroupElement tElement =
        Ed25519Group.ZERO_PRECOMPUTED
            .nullSafeCmov(this.precomputedForSingle[pos][0], ByteUtils.isEqualConstantTime(bAbs, 1))
            .nullSafeCmov(this.precomputedForSingle[pos][1], ByteUtils.isEqualConstantTime(bAbs, 2))
            .nullSafeCmov(this.precomputedForSingle[pos][2], ByteUtils.isEqualConstantTime(bAbs, 3))
            .nullSafeCmov(this.precomputedForSingle[pos][3], ByteUtils.isEqualConstantTime(bAbs, 4))
            .nullSafeCmov(this.precomputedForSingle[pos][4], ByteUtils.isEqualConstantTime(bAbs, 5))
            .nullSafeCmov(this.precomputedForSingle[pos][5], ByteUtils.isEqualConstantTime(bAbs, 6))
            .nullSafeCmov(this.precomputedForSingle[pos][6], ByteUtils.isEqualConstantTime(bAbs, 7))
            .nullSafeCmov(
                this.precomputedForSingle[pos][7], ByteUtils.isEqualConstantTime(bAbs, 8));
    // -16^i |r_i| B
    // noinspection SuspiciousNameCombination
  ...
  /**
   * h = a * B where a = a[0]+256*a[1]+...+256^31 a[31] and B is this point. If its lookup table has
   * not been precomputed, it will be at the start of the method (and cached for later calls).
   * Constant time.
   *
  ...
  public Ed25519GroupElement doubleScalarMultiplyVariableTime(
    ...
    for (; i >= 0; --i) {
      ...
      if (aSlide[i] > 0) {
        tElement =
            tElement.toP3().precomputedSubtract(aGroupElement.precomputedForDouble[aSlide[i] / 2]);
      } else if (aSlide[i] < 0) {
        tElement =
            tElement.toP3().precomputedAdd(aGroupElement.precomputedForDouble[(-aSlide[i]) / 2]);
      }
