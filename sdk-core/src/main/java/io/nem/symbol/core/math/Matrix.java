 ...
 * This class provides default implementations of most matrix functions but they should be optimized
 * in derived classes when performance is important.
 */
public abstract class Matrix {
  ...
  private Matrix join(final Matrix matrix, final boolean isTwoWay, final DoubleBinaryOperator op) {
    ...
    this.forEach(
        (r, c, v) -> result.setAtUnchecked(r, c, op.applyAsDouble(v, matrix.getAtUnchecked(r, c))));

    if (isTwoWay) {
      matrix.forEach(
          (r, c, v) -> result.setAtUnchecked(r, c, op.applyAsDouble(v, this.getAtUnchecked(r, c))));
    }
  ...
  private double aggregate(final DoubleUnaryOperator op) {
    // use a double[1] instead of a double so that the sum can be updated by the
    // lambda
