...
public final class AggregateTransactionBuilder extends TransactionBuilder {
    ...
    protected AggregateTransactionBuilder(final DataInput stream) {
        ...
        try {
            ...
            this.cosignatures = ByteBuffer
                .allocate(getStreamSize() - super.getSize() - payloadSize - 4);
    ...
    public byte[] serialize() {
        return GeneratorUtils.serialize(
            dataOutputStream -> {
                ...
                dataOutputStream
                    .write(this.transactions.array(), 0, this.transactions.array().length);
                dataOutputStream
                    .write(this.cosignatures.array(), 0, this.cosignatures.array().length);
            });
    }
}
