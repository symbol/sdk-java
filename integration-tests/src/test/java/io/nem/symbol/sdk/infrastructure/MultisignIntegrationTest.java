...
class MultisignIntegrationTest extends BaseIntegrationTest {
    ...
    private Account account = config().getNemesisAccount1();
    private Account account2 = config().getNemesisAccount2();
    private Account multisigAccount = config().getNemesisAccount(7);
    private Account cosignAccount1 = config().getNemesisAccount5();
    private Account cosignAccount2 = config().getCosignatory2Account();
    private Account cosignAccount3 = config().getCosignatory3Account();
    ...
    void createMultisigAccount(RepositoryType type) {
        ...
        boolean multisigAccountExist = get(
            getRepositoryFactory(type).createMultisigRepository()
                .getMultisigAccountInfo(multisigAccount.getAddress()).map(s -> true)
        ...
        SignedTransaction signedTransaction = aggregateTransaction
            .signTransactionWithCosigners(
                multisigAccount, Arrays.asList(cosignAccount1, cosignAccount2, cosignAccount3),
                getGenerationHash());

        Transaction transaction = get(
            getTransactionService(type).announce(getListener(type), signedTransaction));
    ...
    void shouldReturnCosignatureAddedViaListener2(RepositoryType type) {
        ...
        TransactionRepository transactionRepository = repositoryFactory
            .createTransactionRepository();

        SignedTransaction signedAggregatedTx =
            createSignedAggregatedBondTransaction(multisigAccount, cosignAccount1,
                account2.getAddress());
                        ...
                        .flatMap(a -> {
                            ...
                            return repositoryFactory.createTransactionRepository()
                                ...
                                .flatMap((page) -> {
                                    ...
                                    AggregateTransaction transactionToCosign = (AggregateTransaction) transactions
                                        .get(0);
                                    ...
                                    return transactionRepository
                                        .announceAggregateBondedCosignature(
                                            cosignatureSignedTransaction).flatMap(r -> {
                                            System.out
                                                .println("announceAggregateBondedCosignature "
                                                    + r);
                                            return listener
                                                .cosignatureAdded(cosignAccount1.getAddress());
                                        });
                                });
                        });
                }));
    ...
    private SignedTransaction createSignedAggregatedBondTransaction(Account multisigAccount,
        Account cosignAccount, Address recipient) {
        ...
        AggregateTransaction aggregateTransaction = AggregateTransactionFactory.createBonded(
            getNetworkType(),
            Collections
                .singletonList(transferTransaction.toAggregate(multisigAccount.getPublicAccount()))
        ).maxFee(maxFee).build();
    ...
    private Observable<Transaction> createHashLockTransactionAndAnnounce(RepositoryType type,
        SignedTransaction signedAggregatedTransaction,
        Account signer) {

        HashLockTransaction hasLockTransaction = HashLockTransactionFactory.create(
            getNetworkType(),
        ...
        SignedTransaction signedLockFundsTransaction = signer
            .sign(hasLockTransaction, getGenerationHash());
        TransactionServiceImpl transactionService = new TransactionServiceImpl(
            getRepositoryFactory(type));
        return transactionService
            .announce(getListener(type), signedLockFundsTransaction);
    }
