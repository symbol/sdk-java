...
class MultisignIntegrationTest extends BaseIntegrationTest {
    ...
    void createMultisigAccount(RepositoryType type) {
        ...
        boolean multisigAccountExist = get(
            getRepositoryFactory(type).createMultisigRepository().getMultisigAccountInfo(multisigAccount.getAddress())
                .map(s -> true).onErrorReturnItem(false));
        if (multisigAccountExist) {
            System.out.println("Multisign account " + multisigAccount.getAddress().plain() + " already exist");
            return;
        }
        MultisigAccountModificationTransaction modifyMultisigAccountTransaction = MultisigAccountModificationTransactionFactory
            .create(getNetworkType(), (byte) 2, (byte) 1,
                Arrays.asList(cosignAccount1.getAddress(), cosignAccount2.getAddress(), cosignAccount3.getAddress()),
                Collections.emptyList()).maxFee(maxFee).build();

        AggregateTransaction aggregateTransaction = AggregateTransactionFactory.createComplete(getNetworkType(),
            Collections.singletonList(modifyMultisigAccountTransaction.toAggregate(multisigAccount.getPublicAccount())))
            .maxFee(maxFee).build();

        SignedTransaction signedTransaction = aggregateTransaction.signTransactionWithCosigners(multisigAccount,
            Arrays.asList(cosignAccount1, cosignAccount2, cosignAccount3), getGenerationHash());
    ...
    void shouldReturnCosignatureAddedViaListener2(RepositoryType type) {
        ...
        SignedTransaction signedAggregatedTx = createSignedAggregatedBondTransaction(multisigAccount, cosignAccount1,
            account2.getAddress());

        Object finalObject = get(
            createHashLockTransactionAndAnnounce(type, signedAggregatedTx, cosignAccount1).flatMap(t -> {
                ...
                return transactionService.announceAggregateBonded(listener, signedAggregatedTx).flatMap(a -> {
                    ...
                    return repositoryFactory.createTransactionRepository().search(
                        new TransactionSearchCriteria(TransactionGroup.PARTIAL)
                            .signerPublicKey(cosignAccount1.getPublicAccount().getPublicKey())).flatMap((page) -> {
                        ...
                        CosignatureTransaction cosignatureTransaction = CosignatureTransaction
                            .create(transactionToCosign);
                        CosignatureSignedTransaction cosignatureSignedTransaction = cosignAccount2
                            .signCosignatureTransaction(cosignatureTransaction);
                        return transactionRepository.announceAggregateBondedCosignature(cosignatureSignedTransaction)
                            .flatMap(r -> {
    ...
    private SignedTransaction createSignedAggregatedBondTransaction(Account multisigAccount, Account cosignAccount,
        Address recipient) {
        TransferTransaction transferTransaction = TransferTransactionFactory
            .create(getNetworkType(), recipient, Collections.emptyList(), PlainMessage.create("test-message")).build();
    ...
    private Observable<Transaction> createHashLockTransactionAndAnnounce(RepositoryType type,
        SignedTransaction signedAggregatedTransaction, Account signer) {

        HashLockTransaction hasLockTransaction = HashLockTransactionFactory
            .create(getNetworkType(), getNetworkCurrency().createRelative(BigInteger.valueOf(10)),
                BigInteger.valueOf(10000), signedAggregatedTransaction).maxFee(maxFee).build();
