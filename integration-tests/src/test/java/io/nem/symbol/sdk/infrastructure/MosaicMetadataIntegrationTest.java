...
public class MosaicMetadataIntegrationTest extends BaseIntegrationTest {
  ...
  public void addMetadataToMosaic(RepositoryType type) {
    ...
    NamespaceId alias =
        setMosaicAlias(
            type, targetMosaicId, "mosaicalias" + targetMosaicId.getIdAsHex().toLowerCase());
    ...
    MosaicMetadataTransaction processedTransaction =
        (MosaicMetadataTransaction) announceCorrectly.getInnerTransactions().get(0);

    Assertions.assertEquals(
        transaction.getTargetMosaicId(), processedTransaction.getTargetMosaicId());
    Assertions.assertEquals(
  ...
  private String assertMetadata(
      MosaicId targetMosaicId, MosaicMetadataTransaction transaction, List<Metadata> metadata) {
            ...
            .filter(
                m ->
                    m.getScopedMetadataKey().equals(transaction.getScopedMetadataKey())
                        && m.getMetadataType().equals(MetadataType.MOSAIC)
  ...
  void searchIntegration(RepositoryType type) {
    ...
    Address sourceAddress =
        Address.createFromEncoded("9852D5EAA9AB038151EEBDD34308B3B2B7D82B92955F298E");
