...
public class MosaicDefinitionTransactionIntegrationTest extends BaseIntegrationTest {
  ...
  void standaloneMosaicDefinitionTransaction(RepositoryType type) {
    ...
    MosaicInfo mosaicInfo =
        get(getRepositoryFactory(type).createMosaicRepository().getMosaic(mosaicId));
    ...
    Assertions.assertTrue(mosaicInfo.isRevokable());
  ...
  private MosaicId createMosaic(RepositoryType type) {
    ...
    Assertions.assertEquals(
        mosaicDefinitionTransaction.getMosaicNonce(), processed.getMosaicNonce());
    return mosaicId;
  }
  ...
  void aggregateMosaicDefinitionTransaction(RepositoryType type) {
    ...
    MosaicDefinitionTransaction processed =
        announceAggregateAndValidate(type, mosaicDefinitionTransaction, this.account).getLeft();
    Assertions.assertEquals(
        mosaicDefinitionTransaction.getMosaicNonce(), processed.getMosaicNonce());
    Assertions.assertEquals(mosaicId, processed.getMosaicId());
  }
