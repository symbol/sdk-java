...
public class ReceiptMappingVertxTest {
    ...
    private final ReceiptMappingVertx mapper = new ReceiptMappingVertx(jsonHelper);

    @Test
    public void getMosaicResolutionStatementHash() {

        List<ResolutionStatementInfoDTO> dtos = jsonHelper
            .parseList(TestHelperVertx.loadResource("Recipient-MosaicResolutionStatement.json"),
                ResolutionStatementInfoDTO.class);

        List<MosaicResolutionStatement> mosaicResolutionStatement = dtos.stream()
            .map(mapper::createMosaicResolutionStatementFromDto).collect(Collectors.toList());

        Assertions.assertEquals("DE29FB6356530E5D1FBEE0A84202520C155D882C46EA74456752D6C75F0707B3",
    ...
    public void getTransactionStatementshash() {
        ...
        List<TransactionStatement> transactionStatements = transactionStatementInfoDTOS.stream()
            .map(mapper::createTransactionStatement).collect(Collectors.toList());
    ...
    public void getTransactionStatements2Recipients() {
        ...
        Assertions
            .assertEquals("85BBEA6CC462B244", ((BalanceTransferReceipt) receipts.get(0)).getMosaicId().getIdAsHex());
        ...
        Assertions
            .assertEquals(NamespaceId.class, ((ArtifactExpiryReceipt) receipts.get(2)).getArtifactId().getClass());

        Assertions.assertEquals(ReceiptType.NAMESPACE_DELETED, receipts.get(3).getType());
        Assertions
            .assertEquals(NamespaceId.class, ((ArtifactExpiryReceipt) receipts.get(3)).getArtifactId().getClass());
    ...
    public void getAddressResolutionStatementsHash() {
        List<ResolutionStatementInfoDTO> dtos = jsonHelper
            .parseList(TestHelperVertx.loadResource("Recipient-AddressResolutionStatement.json"),
                ResolutionStatementInfoDTO.class);

        List<AddressResolutionStatement> addressResolutionStatements = dtos.stream()
            .map(mapper::createAddressResolutionStatementFromDto).collect(Collectors.toList());

        Assertions.assertEquals("9FB5235FB53C815DEFDCC9FC137BF949987097BDE2D3132EEF746F0AE0A6B02F",
