...
public class MathUtils {
    ...
    /**
     * Converts a 2^25.5 bit representation to a BigInteger. Value: 2^EXPONENTS[0] * t[0] +
     * 2^EXPONENTS[1] * t[1] + ... + 2^EXPONENTS[9] * t[9]
     *
     ...
     */
    public static Ed25519EncodedFieldElement reduceModGroupOrder(
    ...
    /**
     * Calculates (a * b + c) mod group order and returns the result. a, b and c are given in 2^8
     * bit representation.
     *
    ...
    public static Ed25519EncodedFieldElement multiplyAndAddModGroupOrder(
        ...
        final BigInteger result =
            toBigInteger(a)
                .multiply(toBigInteger(b))
                .add(toBigInteger(c))
                .mod(Ed25519Group.GROUP_ORDER);
    ...
    /**
     * Gets a random group element in P3 coordinates. It's NOT guaranteed that the created group
     * element is a multiple of the base point.
     *
    ...
    /**
     * Gets a random encoded group element. It's NOT guaranteed that the created encoded group
     * element is a multiple of the base point.
     *
    ...
    /**
     * Creates a group element from a byte array. Bit 0 to 254 are the affine y-coordinate, bit 255
     * is the sign of the affine x-coordinate.
     *
     ...
     */
    public static BigInteger getAffineXFromAffineY(
        final BigInteger y, final boolean shouldBeNegative) {
    ...
    private static BigInteger getSqrtOfFraction(final BigInteger u, final BigInteger v) {
        final BigInteger tmp =
            u.multiply(v.pow(7))
                .modPow(BigInteger.ONE.shiftLeft(252).subtract(new BigInteger("3")), Ed25519Field.P)
                .mod(Ed25519Field.P);
        return tmp.multiply(u).multiply(v.pow(3)).mod(Ed25519Field.P);
    }

    /**
     * Converts a group element from one coordinate system to another. This method is a helper used
     ...
     */
    public static Ed25519GroupElement toRepresentation(
        ...
        switch (g.getCoordinateSystem()) {
            ...
            case CACHED:
                x =
                    gX.subtract(gY)
                        .multiply(gZ.multiply(new BigInteger("2")).modInverse(Ed25519Field.P))
                        .mod(Ed25519Field.P);
                y =
                    gX.add(gY)
                        .multiply(gZ.multiply(new BigInteger("2")).modInverse(Ed25519Field.P))
            ...
            case PRECOMPUTED:
                x =
                    gX.subtract(gY)
                        .multiply(new BigInteger("2").modInverse(Ed25519Field.P))
                        .mod(Ed25519Field.P);
                y = gX.add(gY).multiply(new BigInteger("2").modInverse(Ed25519Field.P))
                    .mod(Ed25519Field.P);
                break;
            default:
        ...
        switch (newCoordinateSystem) {
            case AFFINE:
                return Ed25519GroupElement
                    .affine(toFieldElement(x), toFieldElement(y), Ed25519Field.ONE);
            case P2:
                return Ed25519GroupElement
                    .p2(toFieldElement(x), toFieldElement(y), Ed25519Field.ONE);
            case P3:
                return Ed25519GroupElement.p3(
                    toFieldElement(x),
                    toFieldElement(y),
            ...
            case P1xP1:
                return Ed25519GroupElement.p1xp1(
                    toFieldElement(x), toFieldElement(y), Ed25519Field.ONE, Ed25519Field.ONE);
            case CACHED:
                return Ed25519GroupElement.cached(
                    ...
                    Ed25519Field.ONE,
                    toFieldElement(
                        D.multiply(new BigInteger("2")).multiply(x).multiply(y)
                            .mod(Ed25519Field.P)));
            case PRECOMPUTED:
    ...
    /**
     * Adds two group elements and returns the result in P3 coordinate system. It uses BigInteger
     * arithmetic and the affine coordinate system. This method is a helper used to test the
     * projective group addition formulas in Ed25519GroupElement.
     *
     * @param g1 The first group element.
     * @param g2 The second group element.
     * @return The result of the addition.
     */
    public static Ed25519GroupElement addGroupElements(
        final Ed25519GroupElement g1, final Ed25519GroupElement g2) {
        // Relying on a special coordinate system of the group elements.
        if ((g1.getCoordinateSystem() != CoordinateSystem.P2
            && g1.getCoordinateSystem() != CoordinateSystem.P3)
            || (g2.getCoordinateSystem() != CoordinateSystem.P2
            && g2.getCoordinateSystem() != CoordinateSystem.P3)) {
        ...
        // d = -121665/121666
        final BigInteger dx1x2y1y2 =
            D.multiply(g1x).multiply(g2x).multiply(g1y).multiply(g2y).mod(Ed25519Field.P);
        final BigInteger x3 =
            g1x.multiply(g2y)
                .add(g2x.multiply(g1y))
                .multiply(BigInteger.ONE.add(dx1x2y1y2).modInverse(Ed25519Field.P))
                .mod(Ed25519Field.P);
        final BigInteger y3 =
            g1x.multiply(g2x)
                .add(g1y.multiply(g2y))
                .multiply(BigInteger.ONE.subtract(dx1x2y1y2).modInverse(Ed25519Field.P))
                .mod(Ed25519Field.P);
        final BigInteger t3 = x3.multiply(y3).mod(Ed25519Field.P);

        return Ed25519GroupElement.p3(
            toFieldElement(x3), toFieldElement(y3), Ed25519Field.ONE, toFieldElement(t3));
    }

    /**
     * Doubles a group element and returns the result in the P3 coordinate system. It uses
     * BigInteger arithmetic and the affine coordinate system. This method is a helper used to test
     * the projective group doubling formula in Ed25519GroupElement.
     *
     * @param g The group element.
     * @return g+g.
     */
     ...
     */
    public static Ed25519GroupElement scalarMultiplyGroupElement(
    ...
    public static PublicKey derivePublicKey(final PrivateKey privateKey) {
        ...
        a[31] |= 0x40;
        a[0] &= 0xF8;
        final Ed25519GroupElement pubKey =
            scalarMultiplyGroupElement(Ed25519Group.BASE_POINT, toFieldElement(toBigInteger(a)));
    ...
    public static Signature sign(final KeyPair keyPair, final byte[] data) {
        ...
        final Ed25519EncodedFieldElement r =
            new Ed25519EncodedFieldElement(Hashes.sha3_512(Arrays.copyOfRange(hash, 32, 64), data));
        final Ed25519EncodedFieldElement rReduced = reduceModGroupOrder(r);
        final Ed25519GroupElement R =
            scalarMultiplyGroupElement(Ed25519Group.BASE_POINT,
                toFieldElement(toBigInteger(rReduced)));
        final Ed25519EncodedFieldElement h =
            new Ed25519EncodedFieldElement(
                Hashes.sha3_512(R.encode().getRaw(), keyPair.getPublicKey().getBytes(), data));
        final Ed25519EncodedFieldElement hReduced = reduceModGroupOrder(h);
        final BigInteger S =
            toBigInteger(rReduced)
                .add(toBigInteger(hReduced).multiply(toBigInteger(a)))
                .mod(Ed25519Group.GROUP_ORDER);
