...
public class MustBe {
    ...
    public static void notWhitespace(final String str, final String name, final int maxLength) {
        if (StringUtils.isNullOrWhitespace(str) || str.length() > maxLength) {
            final String message =
                String.format(
                    "%s cannot be null, empty, or whitespace, or have length greater than %d",
                    name, maxLength);
            throw new IllegalArgumentException(message);
        }
    }
    ...
    /**
     * Throws an exception if the specified string does not match the pattern, is empty, or longer
     * than the max length.
     *
     * @param str The string.
     * @param name The string name.
    ...
    public static void match(
        final String str, final String name, final Pattern pattern, final int maxLength) {
        if (null == str
            || str.isEmpty()
    ...
    public static void inRange(
        final int value, final String name, final int minInclusive, final int maxInclusive) {
     ...
     */
    public static void inRange(
        final long value, final String name, final long minInclusive, final long maxInclusive) {
        if (value < minInclusive || value > maxInclusive) {
            final String message =
                String.format("%s must be between %d and %d inclusive", name, minInclusive,
                    maxInclusive);
            throw new IllegalArgumentException(message);
        }
    }
